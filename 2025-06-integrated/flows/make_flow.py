...
1. Validate source, dest, and presence of a metadata file
"""
import json
import logging
...
#######
# Workflow
def get_example_flow(flow_fn, schema_fn):
...
def register_flow(client: FlowsClient, flow_def: dict, schema_def: dict) -> str:
    try:
        resp = client.create_flow("Example flow", flow_def, schema_def, keywords=['apecx', 'apecx-dev'])
...
if __name__ == "__main__":
    ...
    rel = lambda fn: Path(__file__).resolve().parent / fn
    flow_def, schema_def = get_example_flow(
        rel('transfer_with_metadata/flow_definition.json'),
        rel('transfer_with_metadata/input_schema.json'),
    )
